public class Gleitpunktzahl {
    ...
    public void normalisiere() {
         ...
         */
        if(Integer.numberOfLeadingZeros(this.mantisse) < (32 - sizeMantisse)) {

            boolean onlyZeros = true;
            boolean lastbit = false;
            while (Integer.numberOfLeadingZeros(this.mantisse) < (32 - sizeMantisse)) {
                if (lastbit)
                    onlyZeros = false;
                lastbit = (this.mantisse & 1) == 1;
                this.exponent = this.exponent + 1;
                this.mantisse = this.mantisse / 2;
            }


            //System.out.println(this.mantisse);
            if (((this.mantisse & 1) == 1) && onlyZeros == true && lastbit == true) {
                //aufrunden
            ...
            //System.out.println("ret: " + this.mantisse);
        }else if(Integer.numberOfLeadingZeros(this.mantisse) > (32-sizeMantisse)){
            if(this.mantisse == 0){
                this.setNull();
                return;
            }
    ...
    public static void denormalisiere(Gleitpunktzahl a, Gleitpunktzahl b) {
         ...
         *
         *
         *   				AUFPASSEN EXPONENT ZU GROß FÜR MANTISSEN INTEGER
         *   				BEIDE ZAHLEN MÜSSEN GLEICH VIELE NACHKOMMASTELLEN HABEN
         */

        while (a.exponent > b.exponent) {
            a.exponent = a.exponent - 1;
            a.mantisse = a.mantisse * 2;
        }

        while (b.exponent > a.exponent) {
            b.exponent = b.exponent - 1;
            b.mantisse = b.mantisse * 2;
        }
    }
    ...
    public Gleitpunktzahl add(Gleitpunktzahl r) {
        ...
        Gleitpunktzahl ret = new Gleitpunktzahl(r);

        if ((this.isInfinite() && r.isInfinite())) {  //Kann nich plus unendlich und minus unendlich addieren
        ...
        //int numbits = 32 - Integer.numberOfLeadingZeros(this.mantisse);
        //if((32-Integer.numberOfLeadingZeros(r.mantisse)) > numbits)
        //	numbits = 32-Integer.numberOfLeadingZeros(r.mantisse);
        int a = this.mantisse;
        if(this.vorzeichen)
            a = a*-1;
        int b = r.mantisse;
        if(r.vorzeichen)
            b = b*-1;
        ret.mantisse = a + b;
        ret.vorzeichen = ret.mantisse < 0;
        ret.mantisse = Math.abs(ret.mantisse);
        ret.exponent = this.exponent;
        //System.out.println("--------" + ret.mantisse);
        ret.normalisiere();
        return ret;
    }
